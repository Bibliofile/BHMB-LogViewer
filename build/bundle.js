...
var html = "<link rel=\"stylesheet\" href=\"https://cdn.jsdelivr.net/npm/flatpickr/dist/flatpickr.min.css\">\r\n<template>\r\n  <li>\r\n    <span class=\"t\"></span>\r\n    <span class=\"m\"></span>\r\n  </li>\r\n</template>\r\n<div id=\"logviewer\" class=\"container\">\r\n  <h3 class=\"title is-3\">Options</h3>\r\n  <div class=\"columns\">\r\n    <div class=\"column\">\r\n      Starting date:\r\n      <input class=\"input\" data-for=\"start\" />\r\n    </div>\r\n    <div class=\"column\">\r\n      Ending date:\r\n      <input class=\"input\" data-for=\"stop\" />\r\n    </div>\r\n    <div class=\"column\">\r\n      Search:\r\n      <input class=\"input\" data-for=\"search\" placeholder=\"Search...\" />\r\n      <h4 class=\"title is-4\">Tips:</h4>\r\n      <ul>\r\n        <li>You can use <code>online=NAME</code> to show chat from when someone is online</li>\r\n        <li>You can use <code>sender=NAME</code> to show chat from NAME</li>\r\n        <li>You can use <code>context=#</code> to show <code>#</code> messages around matched messages</li>\r\n        <li>To use names with spaces, remove the space.</li>\r\n        <li>Search is case-insensitive.</li>\r\n        <li><code>online=NAME1 online=NAME2</code> will show chat when <b>both</b> NAME1 and NAME2 are online.</li>\r\n        <li><code>sender=NAME1 sender=NAME2</code> will show chat from both NAME1 and NAME2.</li>\r\n      </ul>\r\n    </div>\r\n  </div>\r\n\r\n  <ol style=\"list-style-type: none\"></ol>\r\n</div>";
...
function parse(search) {
    let ret = { search: [], online: [], sender: [], context: 0 };
    search.toLocaleLowerCase().split(/\s/).filter(Boolean).forEach(word => {
        if (word.startsWith('online='))
            ret.online.push(word.substr(7));
        else if (word.startsWith('sender='))
            ret.sender.push(word.substr(7));
        else if (word.startsWith('context='))
            ret.context = Math.abs(parseInt(word.substr(8), 10)) || 0;
        else
            ret.search.push(word);
    });
    ret.online = ret.online.map(v => v.toLocaleUpperCase());
    ret.sender = ret.sender.map(v => v.toLocaleUpperCase());
    return ret;
}
function findIndexes(arr, checker) {
    let indexes = [];
    arr.forEach((el, index) => checker(el, index, arr) && indexes.push(index));
    return indexes;
}
bot.MessageBot.registerExtension('bibliofile/logs', (ex) => __awaiter(undefined, void 0, void 0, function* () {
    const ui = ex.bot.getExports('ui');
    if (!ui)
        return;
    let tab = ui.addTab('Logs');
    tab.innerHTML = html;
    const query = (selector) => tab.querySelector(selector);
    let logs = yield ex.world.getLogs(true);
    ...
    const render = () => {
        ...
        let search = query('[data-for=search]');
        ...
        let matchIndexes = findIndexes(logs, ({ message }, index) => {
            ...
            if (joinMatch)
                online.add(joinMatch[1].replace(/\s/g, ''));
            let leaveMatch = message.match(/ - Player Disconnected (.*)$/);
            if (leaveMatch)
                online.delete(leaveMatch[1].replace(/\s/g, ''));
            return [
        ...
        let toRender = logs.filter((_, index) => {
            if (index >= matchIndexes[currentMatchIndex] + parsed.context + 1)
                currentMatchIndex++;
    ...
    let pickers = [
        flatpickr(query('[data-for=start]'), Object.assign({}, pickerOptions, { defaultDate: startDate, onChange([date]) { startDate = date; debouncedRender(); } })),
        flatpickr(query('[data-for=stop]'), Object.assign({}, pickerOptions, { defaultDate: endDate, onChange([date]) { endDate = date; debouncedRender(); } })),
    ];
